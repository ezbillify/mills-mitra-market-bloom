...
import { Package, Truck, Calendar, User, MapPin, Phone, Mail, Hash, DollarSign } from "lucide-react";
...
const OrderDetailsDialog = ({ orderId, open, onOpenChange, onUpdateStatus }: OrderDetailsDialogProps) => {
  ...
  const calculateItemsTotal = () => {
    return orderItems.reduce((sum, item) => sum + (item.quantity * Number(item.price)), 0);
  };
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {orderDetails.profiles?.address && (
                          <>
                            {orderDetails.profiles.address}<br />
                            {orderDetails.profiles.city && `${orderDetails.profiles.city}, `}
                            {orderDetails.profiles.postal_code && `${orderDetails.profiles.postal_code}`}<br />
                            {orderDetails.profiles.country}
                          </>
                        )}
                        {!orderDetails.profiles?.address && 'No address on file'}
                      </p>
                    </div>
                  </div>
                </div>
