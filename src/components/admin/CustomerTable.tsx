...
const CustomerTable = ({ customers, onViewCustomer }: CustomerTableProps) => {
  ...
  const displayCustomerName = (customer: Customer) => {
    console.log(`📛 Displaying name for customer ${customer.id.substring(0, 8)}: "${customer.name}"`);
    
    // Use the name that was generated by the utility function
    if (customer.name && customer.name.trim() !== '' && !customer.name.startsWith('Customer ')) {
      console.log(`✅ Using processed name: "${customer.name}"`);
      return customer.name;
    }
    
    // If the processed name is still a fallback, try to build from profile data
    if (customer.profile?.first_name || customer.profile?.last_name) {
      ...
      if (fullName) {
        console.log(`🔧 Built name from profile: "${fullName}"`);
        return fullName;
      }
    }
    
    // If we still don't have a good name, use email prefix
