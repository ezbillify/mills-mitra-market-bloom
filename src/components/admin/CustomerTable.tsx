...
const CustomerTable = ({ customers, onViewCustomer }: CustomerTableProps) => {
  ...
  const displayCustomerName = (customer: Customer) => {
    ...
    // Use the name that was already generated by the utility function
    // Only fall back to manual generation if the utility failed completely
    if (customer.name && customer.name.trim() !== '') {
      console.log(`✅ Using processed name: "${customer.name}"`);
      return customer.name;
    }
    
    // Emergency fallback only if utility function completely failed
    const fallbackName = `Customer ${customer.id.substring(0, 8)}`;
    console.log(`🚨 Emergency fallback: "${fallbackName}"`);
    return fallbackName;
  };
